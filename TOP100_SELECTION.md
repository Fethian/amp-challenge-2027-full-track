...
changed during repository preparation. The V7.1 audit arm `G6_V5_true` is
identical to `frozen_top100.fasta` in all 100 sequences and rank order.
...
candidate-population percentiles and fused 50/50 for ranking. This differs from
the evaluation fusion, which directly averaged calibrated probabilities.
...
In each Random-25 draw, 25 distinct rows were sampled, each scenario was
averaged over those rows, and the minimum scenario mean was recorded. The
aggregation order is therefore `min_s mean_i utility(i,s)`, not `mean_i min_s
utility(i,s)`.
...
0.8201/0.8171/0.8152, versus 0.8094/0.8035/0.7987 for the mixed
OmegAMP-HydrAMP branch. The mixed branch therefore failed the prespecified
promotion rule (non-negative P10 delta and at least +0.005 mean delta).

The V7.1 closing audit was a later, fixed-scope ARCADIAMP challenge. It compared
ARCADIAMP/OmegAMP Top100 ratios 0/100, 25/75, 50/50, 55/45, 75/25, and 100/0
against G6 V5 using 100,000 Random-25 draws, pairing sensitivity, and external
activity and safety diagnostics. ARCADIAMP was not part of the earlier
117,957-row OmegAMP/HydrAMP pool. The matched 0/100 comparator is not G6 V5;
generator proportions alone do not establish sequence identity. Every
challenger failed at least the fixed
external-evidence and safety gates; G6 V5 was retained.
...
The five closing gates were: hard sequence/release constraints; Phase-1
improvement without excess degradation; positive broad and MDR paired-bootstrap
confidence-interval lower bounds; favorable leave-one-model-family-out
direction; and no increase in double-red safety-proxy fraction. These were not
the HydrAMP gate above. “One-draw failure probability” means a proxy-utility
delta at or below zero for one sampled 25-member portfolio, not a wet-lab
failure probability.
